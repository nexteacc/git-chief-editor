...
// HighlightListCard - variant for displaying achievement lists
export interface HighlightListCardProps {
  title: string;
  items: string[];
  icon: React.ReactNode;
  color?: ColorTheme;
...
export const HighlightListCard = React.forwardRef<HTMLDivElement, HighlightListCardProps>(
  ({ title, items, icon, color = "default", className }, ref) => {
    const theme = colorThemes[color] || colorThemes.default;
    ...
    return (
      ...
      >
        {/* Bookmark shape */}
        <div className="absolute top-0 right-6 h-16 w-12 bg-white/95 backdrop-blur-sm [clip-path:polygon(0%_0%,_100%_0%,_100%_100%,_50%_75%,_0%_100%)]">
          <div
            className="absolute inset-0 flex items-center justify-center"
        ...
        <div className="flex h-full flex-col">
          {/* Title */}
          <motion.h3 variants={itemVariants} className="text-xl font-bold tracking-tight mb-4">
            {title}
          </motion.h3>

          {/* Divider */}
          <motion.div variants={itemVariants} className="h-px w-full bg-white/20 mb-4" />
          ...
          <ul className="space-y-3">
            {items.map((item, idx) => (
              ...
              >
                <span className="inline-block w-1.5 h-1.5 rounded-full bg-white/80 mt-2 mr-3 flex-shrink-0 group-hover:bg-white transition-colors" />
                <p className="text-sm leading-relaxed opacity-95">{item}</p>
              </motion.li>
            ))}
          </ul>
        </div>
...
HighlightListCard.displayName = "HighlightListCard";